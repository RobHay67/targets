...
def render_retained_fundraisers(scope):
	...
	for region in scope.target_columns:
		...
		if region == 'row_heading':
			...
			with col1: 
				...
				st.write('')
				st.write('**'+region+'**')
			with col2: render_prior_regos(scope, region)
			with col3: render_regos_widget(scope, region)
			with col4: render_active_widget(scope, region)
			with col5: render_apam_widget(scope, region)
	...
	previous_campaign = scope.campaign - 1
	...
	for region in scope.target_columns:
		...
		if region == 'row_heading':
			...
			with col1: st.markdown(format_string(region, align='Right', bold=False), unsafe_allow_html=True)
			with col2: st.markdown(format_regos(rates['regos_total_prior'], align='right'), unsafe_allow_html=True)		# this is not needed
			with col3: st.markdown(format_regos(rates['regos'], align='right'), unsafe_allow_html=True)              # This will be the retained number
			with col4: st.markdown(format_regos(rates['active'], align='right'), unsafe_allow_html=True)
			with col5: st.markdown(format_dolls(rates['apam'], align='right'), unsafe_allow_html=True)
			with col6: st.markdown(format_dolls(rates['funds'], align='right'), unsafe_allow_html=True)
			with col7: st.markdown(format_percent(retention_ratio, align='Center'), unsafe_allow_html=True)
			with col8: st.markdown(format_percent(active_ratio, align='Center'), unsafe_allow_html=True)
...
def tenure_headings(scope, campaign, col1,col2,col3,col4,col5,col6,col7,col8,col9):
	...
	with col2: st.markdown(format_string(('Total Registrations ' + previous_campaign) ,align='Right'), unsafe_allow_html=True)
	with col3: st.markdown(format_string(('Total Retained into ' + current_campaign) ,align='Right'), unsafe_allow_html=True)
	with col4: st.markdown(format_string('Active Registrations' ,align='Right'), unsafe_allow_html=True)
	with col5: st.markdown(format_string('APAM' ,align='Right'), unsafe_allow_html=True)
